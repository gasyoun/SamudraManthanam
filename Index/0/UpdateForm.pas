...
uses
  Forms, Controls, StdCtrls, ComCtrls, Classes, SysUtils,
...
begin
  ...
  try
    FHttpClient.OnDataReceived := HttpDataReceived;
    FHttpClient.AllowRedirect := True;
    lblStatus.Caption := 'Подключение...';
    Application.ProcessMessages;
    FHttpClient.Get(UpdateURL, FileStream);
    if not FCancelled then
    begin
      lblStatus.Caption := 'Загрузка завершена!';
      // Запускаем POUpdater и закрываем программу
      ShellExecute(0, 'open',
                   PChar(AppPath + 'POUpdater.exe'),
                   nil, nil, SW_SHOW);
