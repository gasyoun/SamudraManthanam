...
// Вставка
function Select(searchTerm) {
  highlightSearchTerm(searchTerm);
}

function highlightSearchTerm(searchTerm) {
  const contentDiv = document.getElementById("content");
  contentDiv.dataset.searchTerm = searchTerm;
  const contentHTML = contentDiv.innerHTML;
  const highlightedContent = contentHTML.replace(new RegExp(searchTerm, 'gi'), '<span class="highlight">$&</span>');
