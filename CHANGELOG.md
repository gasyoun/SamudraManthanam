_Created: 25-08-2026 · Last updated: 24-09-2026_
...
### Added
- **H5426 (Opus, E016 wave 6) — DOCUMENTATION_INDEX.md coverage is generated, not hand-maintained (24-09-2026).** The index (last hand-touched 14-08-2026) named 35 of the 111 Markdown files under the repo root and `docs/`, so 76 docs were invisible to anyone orienting from it. New stdlib-only [`scripts/documentation_index_check.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/scripts/documentation_index_check.py) keeps the hand-written current/supporting/historical sections hand-owned and generates one *Every other Markdown file* section below them listing, with its H1 title, every doc those sections do not name. Prove: `python scripts/documentation_index_check.py --check` → `PASS: every scanned doc appears in the index (76 via the generated section)`, exit 0; before the fix the same command exited 1 naming all 76.
- **H5281 (Opus) — НКРЯ showcase package: real НКРЯ metadata fields, rights-table fix, sphere + spot-check sheet (23-09-2026).** [`nkrya_export.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/web/corpus_builder/nkrya_export.py) v0.2.0 writes the НКРЯ parallel-corpus header fields only (`headers_all, authors_all, created, translator, lang_orig, date_trans, sphere, words`; mapping comment in code, old names gone from header + TMX props) from a three-layer merged meta (desktop `Index/lib/x86_64-win64/Data/<slug>.html.meta.json` ← curated sidecar ← new [`nkrya_showcase_bib.json`](https://github.com/gasyoun/SamudraManthanam/blob/main/web/corpus_builder/nkrya_showcase_bib.json), 24 showcase texts with translator/date_trans/publisher/created/sphere + evidence URLs). [`RIGHTS_TABLE.md`](https://github.com/gasyoun/SamudraManthanam/blob/main/nkrya-parallel/export/RIGHTS_TABLE.md) re-rendered over the frozen Wave-4 set: translator coverage **130/131** (was 4/131 — the builder read the wrong meta store; the H821 «metadata loss» never happened). New [`build_nkrya_showcase_package.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/web/corpus_builder/build_nkrya_showcase_package.py): 24 texts, 28,440 pairs, **24/24 match** FULL_CORPUS_VALIDATION, commentary excluded, IAST+SLP1, inline DCS `<ana/>` + CC BY 4.0 header note, deterministic zips (full + public-domain-only) as a GitHub **draft** release asset; committed metadata only in [`nkrya-parallel/export/showcase/`](https://github.com/gasyoun/SamudraManthanam/tree/main/nkrya-parallel/export/showcase). One 10-card review sheet via [`scripts/build_nkrya_showcase_sheet.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/scripts/build_nkrya_showcase_sheet.py) (3 sphere + 7 seeded spot-checks, every row source-verified; evidence gate PASS). samskrtam.ru page draft: [SAMSKRTAM_CORPUS_LINGUIST_PAGE_DRAFT_NKRYA_SHOWCASE_23-09-2026.md](https://github.com/gasyoun/SamudraManthanam/blob/main/docs/SAMSKRTAM_CORPUS_LINGUIST_PAGE_DRAFT_NKRYA_SHOWCASE_23-09-2026.md) (not deployed). Prove: `PYTHONPATH=. python -m pytest tests/test_nkrya_export.py` (23 passed, 5 skipped; corpus 9 passed).
- **NKRYa parallel-corpus submission decisions + letter drafts (23-09-2026).** Live API probe: NKRYa has Hindi (9 texts, 9,490 sentences) but no Sanskrit; its real parallel-text metadata fields (`translator`, `authors_all`, `created`, `date_trans`, `lang_orig`, `sphere`) differ from the exporter's guessed ones; RIGHTS_TABLE misses translators the meta files already carry (127/131 blank vs 204/231 credited). MG rulings (write now, showcase first, quota letter) in [DECISIONS_NKRYA_PARALLEL_SUBMISSION_23-09-2026.md](https://github.com/gasyoun/SamudraManthanam/blob/main/docs/DECISIONS_NKRYA_PARALLEL_SUBMISSION_23-09-2026.md); letters in [LETTER_DRAFT_NKRYA_SANSKRIT_PARALLEL_AND_API_QUOTA_23-09-2026.md](https://github.com/gasyoun/SamudraManthanam/blob/main/docs/LETTER_DRAFT_NKRYA_SANSKRIT_PARALLEL_AND_API_QUOTA_23-09-2026.md); work → Uprava H5281.
- **H4715 (OxAlpha) — Cyrillic proper-noun SLP1 keys joined into the five name glossaries** (kosha dataset `cyrillic-proper-noun-slp1`, xwalk-a10). Deterministic builder [`scripts/build_name_glossary_slp1_join.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/scripts/build_name_glossary_slp1_join.py) keys glossary headwords of Рамаяна 3 / Гринцер (Рамаяна I–II, Бада Кадамбари) / Потапова with their validated SLP1 forms from SanskritLexicography `cyrillic_proper_noun_slp1.tsv` (H3985; `rule_derived_keys: 0` invariant re-derived per row): **965 joined rows / 918 distinct headword hits over 5,114 entries; 506 of 518 table forms have a glossary home; validation tiers onomasticon 854 / lexicon 63 / iast-witness-only 48**; Смирнов stays honestly unkeyed (no IAST-witness seeds). Outputs: [`reports/name_glossary_slp1_joined.tsv`](https://github.com/gasyoun/SamudraManthanam/blob/main/reports/name_glossary_slp1_joined.tsv) + [`reports/name_glossary_slp1_join_report.json`](https://github.com/gasyoun/SamudraManthanam/blob/main/reports/name_glossary_slp1_join_report.json) — identifiers/keys/metadata only, Russian gloss text NOT redistributed (in-copyright editions). 15-name sample verify 15/15 PASS (headword provenance, slp1 re-derived from IAST witness via sanscript, inline-paren agreement with NOT_IAST_SPAN class for Cyrillic synonym spans). Prove: `python3 scripts/build_name_glossary_slp1_join.py --check` → CHECK PASS (byte-parity, timestamp-tolerant). **Rev 2 (independent-verifier fix, same day):** parenthesised spans containing ANY Cyrillic (Потапова-style Russian gloss/synonyms, editorial «или …»/etymology notes) are suppressed from `iast_inline` entirely — identifiers only, 0 Cyrillic characters outside the `cyrillic` column (machine-checked); 15-name sample now re-reads source glossary lines to confirm headword presence instead of asserting it.
...
### Fixed
- **nkrya-parallel npm CI invariant green again: 8 vulnerabilities (2 high) → 0 (23-09-2026).** The CI job "npm build and high-severity audit" had been red on `main` for ~21 days on new advisories against transitive deps of Docusaurus 3.10.2. Fixed with plain `npm audit fix` (no `--force`, no `package.json` change, lockfile only, all semver-compatible): `browserslist` 4.28.6→4.29.0 (high: GHSA-c83g-rgw3-j3cx, GHSA-73wf-gq98-2v4g), `svgo` 3.3.4→3.3.5 (high: GHSA-w27v-7q3p-w38r, GHSA-4vpr-x523-8j87), `joi` 17.13.4→17.13.8 (GHSA-6w3j-5fw6-r9vr, GHSA-gg4h-3hg2-grpc), `qs` 6.15.3→6.16.0 + `body-parser` 1.20.6→1.20.8 + `express` 4.22.2→4.22.3 (GHSA-x5fp-wj9c-mxmx, GHSA-4mjr-xmp4-gh2g), `colord` 2.9.3→2.10.0 (GHSA-2wm5-q62r-hmrv), `baseline-browser-mapping` 2.10.43→2.11.25 (GHSA-w5vr-8v7q-w6rv). Prove (in `nkrya-parallel/`): `npm ci && npm audit --omit=dev --audit-level=high && npm run build` → exit 0 each, "found 0 vulnerabilities"; full audit incl. dev deps also 0.
...
### Fixed
- **H3614 (OxAlpha) — the 47 suffix_depth violations behind the red Duplicate-suffix corpus invariant repaired at the root (H3552 residual, MG ruling 28-08-2026 "chinit'").** Root cause: the legacy Ignatiev exporter anchored interleaved front-matter/essay chunks to the *upcoming* verse anchor and disambiguated with a bare `chr(ord(c)+1)` letter run — `brihannila-tantra:22.2b..22.2y` and `shaktisangama-tantra:37.2b..37.2\x86` (overflowing past `z` into punctuation/C1 controls that a `[b-z]` gate class cannot even see). Four-layer repair, invariant strengthened never weakened: (1) all three collision mints ([`ignatiev_book_to_canonical.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/web/corpus_builder/ignatiev_book_to_canonical.py), [`ignatjev_pdf_to_canonical.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/web/corpus_builder/ignatjev_pdf_to_canonical.py), `html_to_canonical.SourceIDState`) cap at `z` and fall back to a LINE_ID_SCHEME §C prose id, reporting `id_collision_overflow`; (2) `dup_suffix_report`'s `DUP_ID_RE` suffix class widened to the full debris space `[b-z{|}~\x7f-\x9f]` (`'a'` stays excluded — GRETIL pada letter); (3) new deterministic, idempotent [`web/scripts/fix_dup_suffix_runs.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/web/scripts/fix_dup_suffix_runs.py) re-keys each debris run — the run's last member restored to the bare verse root (`22.2y→22.2`, `37.2\x86→37.2`), earlier members to `c{chapter}.p{n}` prose ids: 63 ids re-keyed, zero content dropped, manifest pin rebuilt + validated (235 sources / 744,151 records); (4) the regex endpoint's row-by-row aiosqlite hop (~1.2 s of the H1926 2 s hard deadline spent on transport for 3.6k rows) now fetches in batches of 256 with identical order/stats/deadline semantics — removes the pre-existing order-dependent `test_golden_query_regex` red on main. Prove: `python web/scripts/dup_suffix_report.py --strict` → exit 0 (235 suffixed ids, only `b`); full corpus gate `DB_PATH=… python web/scripts/run_corpus_tests.py` → **PASS 45/45** (red with 47 violations since ≤13-08-2026, v0.19.41..v0.19.47); hermetic suite 1028 passed.
...
### Fixed
- **`a41_stats.py` aborted from any clean checkout (H2403).** The `web/corpus.db` probe — which feeds one *reconciliation footnote* — crashed the whole recompute with `no such table: sources`, because that 742 MB runtime view is gitignored and resolves to a 0-byte stub in a fresh clone or linked worktree. That silently falsified the paper's "every statistic recomputes in one pass" claim. The probe now degrades to null fields with a stderr notice, and the headline recount was re-verified in a fresh worktree.
### Changed
- **A41 extras census re-measured, grown 18× (H2403).** The corpus of record is **byte-stable a fourth time** (574,939 records / 574,939 unique IDs; verse cardinality 78,139 · 10,009 · 80 over 88,228 groups — identical to 26-06, 08-07, 11-07), but `web/corpus_builder/jsonl/` now holds **269 files with 121 post-report extras / 199,379 records** (was 7 / 11,056 on 11-07). None enters any reported figure; §3.1, §11 row 1, the Data section and the data statement now state the real numbers and flag fold-or-formally-exclude as the largest freeze-time decision. Also corrected the stale "mint a Zenodo DOI" gate: a **software** concept DOI already exists ([10.5281/zenodo.21317315](https://doi.org/10.5281/zenodo.21317315)) — what is owed is a **dataset** DOI.
