...
### Fixed
- **Cyrillic leakage purged from verse-Sanskrit (`#sa`) fields** — 2,809 of the
  78,220 `#sa` segments carried editorial artifacts that do not belong in an IAST
  text field: 2,796 `БхГ X.Y` Bhagavad-Gītā cross-reference markers (4 Gītā
  sources + Bhīṣmaparva), 6 `{Проверить!}` / `[…GRETIL…]` notes, and **5 genuine
  homoglyph-corrupted words** where a Cyrillic letter masqueraded as Latin
  (`saṃcukoсa`→`saṃcukoca`, `сalāgra…`/`сekṣvāku…`/`сhīlavān` with Cyrillic `с`,
  and `tad-vipākа-аnuguṇānām` with Cyrillic `а`, in Rām. Sundarakāṇḍa + Yoga-sūtra
  4.8). Markers/notes deleted; homoglyphs **transliterated** (с→c, а→a), never
  deleted. Source jsonl cleaned via `web/ingest/clean_sa_cyrillic.py`; the
  homoglyph inventory is `web/ingest/HOMOGLYPH_WORDS.tsv` (generated by
  `scan_homoglyphs.py`); a deployed `corpus.db` is patched in place — without a
  full re-ingest — by `web/ingest/fix_corpus_db_sa.py --db <path> --apply`
  (idempotent; `corpus_lines` is fts5 so the search index updates automatically).
  The 78,574 mixed-script tokens in *Russian* fields (e.g. Vasmer's `*Dunajь`,
  `&quot;огонь&quot`) are legitimate and were left untouched.
