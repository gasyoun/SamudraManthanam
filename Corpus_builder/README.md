...
книги; границы книг помечаются настраиваемым `BookSign` (задаётся в
...
отделяет текст шлоки от её номера в строках санскрита.
...
2. **Сборка HTML** ([`uMhHTML.pas`](https://github.com/gasyoun/SamudraManthanam/blob/main/Corpus_builder/PSRCBuilder/uMhHTML.pas),
   ...
   проверяет перекрёстные ссылки и по главам/шлокам пишет HTML.
