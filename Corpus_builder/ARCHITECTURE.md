...
(там — этапы и очерёдность; здесь — структура и зависимости) и
...
- **GUI ↔ движок.** Многокнижная оркестрация (`PrepareBook`, склейка) живёт в
  `fMainForm`, а не в движке, — её нельзя запустить без формы. → В целевой
...
- **Отчёт об ошибках — только текст.** `ErrList` → `Err.txt`. → Плюс
...
4. **Кодировки — один UTF-8 слой.** Ввод-вывод через `lazUTF8`; предположение о
   CP-1251 уходит. Санскрит остаётся в широком типе, но обёрнут единообразно.
5. **Отчёт — структурируемый.** `Validator`/`Report` отдают ошибки и текстом
...
- **Выход ядра:** HTML-текст (в поток) + объект отчёта (ошибки/предупреждения).
  Побайтовая совместимость с текущим выходом — критерий приёмки переноса.
- **Стабильно наружу:** формат входных `.txt` (см. README) и формат имён/маркеров
...
|---|---|
| Golden-тесты, структурированный отчёт | Фаза 0 |
| Ядро без VCL, `{$MODE Delphi}`, UTF-8 слой | Фаза 1 |
| Один источник утилит (`Units/`) | Фаза 2 |
| `.lpi`/`.lpr`, формы VCL→LCL, приёмка эталоном | Фаза 3 |
| Оркестрация в ядре, абстрактный поток, CLI | Фаза 4 |
| Судьба GUI (LCL vs CLI-only) | Фаза 5 |

Порядок реализации архитектуры совпадает с очерёдностью фаз: сперва «сеть
