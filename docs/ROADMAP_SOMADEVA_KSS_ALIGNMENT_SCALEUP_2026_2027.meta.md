...
1. **P0 Russian inventory** — replace the "unverified for books 11–18" gap with a real coverage table. Highest-value, gates the rest.
2. **Replace the §3 LLM projection with a measured number** once the P2 pilot runs (currently a forecast from mw_ru/pwg_ru throughput).
3. Decide śloka-rekey policy for books 1–10 (P4) to avoid mixed-keying search UX.
4. Verify the provisional per-lambaka bibliography before НКРЯ submission.

## Limitations
- The LLM throughput is projected, not measured (stated honestly in §3).
- Books 1–10 are sentence-keyed (śloka anchoring lost upstream); the roadmap plans recovery but does not yet do it.
- Russian availability for books 11–18 is unknown — the whole 11–18 arm is gated on P0.
