...
| 17-07-2026 | Opus 4.8 (`claude-opus-4-8`) | Added **§10 (runtime & the 2026-07 speedup)** — the port's epithet layer went flat-`re`-alternation → Aho-Corasick ([`_aho.py`](https://github.com/gasyoun/SamudraManthanam/blob/main/web/corpus_builder/sanskritisms/_aho.py)), byte-identical, **3.1×** on MBh Āraṇyakaparva; plus output-preserving hot-path fixes in all three notebooks (`open_files`/`search`/`index_unite`/`get_wordforms`/`capital_search`, `translate` memoize). Before/after table + root-cause in §10. [H1204](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1204-Opus_SamudraManthanam_rubanova-nkrya-speedup_17.07.26.md). |
